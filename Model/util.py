...
cmd_opt.add_argument('-data(ori)', default=None, help='data(ori) folder name')
...
cmd_opt.add_argument('-test_number', type=int, default=0, help='if specified, will overwrite -fold and use the last -test_number graphs as testing data(ori)')
...
class GNNGraph(object):
    def __init__(self, g, label, node_tags=None, node_features=None):
        ...
        if len(g.edges()) != 0:
            ...
            self.edge_pairs[:, 0] = x
            self.edge_pairs[:, 1] = y
            self.edge_index = self.edge_pairs.T
...
def load_data():
    ...
    print('loading data(ori)')
    g_list = []
    label_dict = {}
    feat_dict = {}

    with open('data(ori)/%s/%s.txt' % (cmd_args.data, cmd_args.data), 'r') as f:
    ...
    if cmd_args.test_number == 0:
        train_idxes = np.loadtxt('data(ori)/%s/10fold_idx/train_idx-%d.txt' % (cmd_args.data, cmd_args.fold), dtype=np.int32).tolist()
        test_idxes = np.loadtxt('data(ori)/%s/10fold_idx/test_idx-%d.txt' % (cmd_args.data, cmd_args.fold), dtype=np.int32).tolist()

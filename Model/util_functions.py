...
def genenet_attribute(allx,tfNum):
    ...
    allx_ = StandardScaler().fit_transform(allx)
    #mean1=allx_.mean(axis=0)
    #std1=allx_.std(axis=0)
    trainAttributes = np.average(allx_, axis=1).reshape((len(allx),1))
...
def genenet_attribute_feature(allx,tfNum,searchNum):
    ...
    #allx_=allx_.T
    #bb = allx_.std(axis=1)
    #allx_0 = max_min_normalization(allx)
    #allx_ = MinMaxScaler().fit_transform(allx_)
...
# Negative sampling of the data(ori), not restrict on TF
...
def add_gauss_noise(allx,μ=0, sigma=1):
    # 默认为原始数据添加μ为0，sigma为1的高斯噪声
    allx_noise = np.zeros(allx.shape)
    random.seed(1)
    for i in range(allx.shape[1]):
        gauss_noise=np.zeros(allx[:,i].shape) # 生成高斯噪声
        for j in range(len(allx[:,i])):
            p=allx[:,i][j]/np.sum(allx[:,i]) #第(i,j)个数据的高斯噪声的权重
            gauss_noise[j] = p*random.gauss(μ, sigma)
        allx_noise[:, i] = allx[:, i] + gauss_noise
    return allx_noise
...
def genenet_attribute_feature_noise(allx, tfNum, searchNum, μ=0.0,sigma=0.1):
    # 1: average to one dimension
    allx_ = StandardScaler().fit_transform(allx)
    allx_ = max_min_normalization(allx_)
...
def max_min_normalization(allx):
    #所有的数据 x 分布在 [0 ， 1] 之间
...
def extractLinks2subgraphs_test(Atest, test_pos, test_neg, h=1, max_nodes_per_hop=None, test_node_information=None):
    # automatically select h from {1, 2}
    if h == 'auto': # TODO
        # split train into val_train and val_test
        _, _, val_test_pos, val_test_neg = sample_neg(A, 0.1)
